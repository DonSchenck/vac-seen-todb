...
The first step is to create an instance of a PostgreSQL database. This database is required by the Marten event store that I'm using in my C# application.  
...
dschenck@DSCHENCK-W10E:~/src/github/donschenck/sandbox-cleaner$ oc get templates --namespace=openshift  | grep postgresql
eap72-postgresql-persistent-s2i                     An example Red Hat JBoss EAP 7 application with a persistent PostgreSQL datab...   37 (16 blank)     11
eap72-postgresql-s2i                                An example Red Hat JBoss EAP 7 application with an PostgreSQL database config...   36 (16 blank)     10
jws31-tomcat7-postgresql-persistent-s2i             An example JBoss Web Server application with a PostgreSQL database. For more...    28 (10 blank)     10
jws31-tomcat7-postgresql-s2i                        Application template for JWS PostgreSQL applications built using S2I.              27 (10 blank)     9
...
Get info for the template I need:  
...
With this information in hand, the following command will create exactly what I want — a persistent PostgreSQL instance in my project in my cluster:  

Run the command to create the PostgreSQL instance:
...
Environment variable MARTEN_CONNECTION_STRING is to connect to the PostgreSQL database. This connection string is provided to the application by using a Secret.
...
The secret is contained in the file "vac-seen-todb-connection-string-secret.yaml". 
