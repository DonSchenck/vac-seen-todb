...
invoke-expression "oc process vac-seen-todb-cronjob-template SERVICE_ACCOUNT_SECRET_NAME=$SERVICE_ACCOUNT_SECRET_NAME BOOTSTRAPSERVERS=$BOOTSTRAPSERVERS SASL_MECHANISM=$SASL_MECHANISM SECURITY_PROTOCOL=$SECURITY_PROTOCOL" | oc create -f -